        ...
        <main>
            ...
            <div className='flex gap-5 mt-10 '>
               {tutorailsData.map((item)=>(
                    <div key={item.id} className='flex flex-col gap-1 hover-scale-105  bg- cursor-pointer bg-[#111827] border border-gray-800 rounded-2xl p-6 hover:border-blue-600 transition'>
                    <h2>{item.title}</h2>
                    <p className='text-gray-500 '>{item.description}</p>
                    <a href={item.link} className='text-blue-600'>Open Resource →</a>
          </div>
